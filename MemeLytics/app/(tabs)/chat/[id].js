...
const RefreshSpinner = ({ isDark, refreshing }) => {
  ...
  // Стили внутри компонента (идентично index.js)
  const spinnerStyles = {
    refreshSpinner: {
      width: 48,
      height: 48,
...
const ChatScreen = () => {
 ...
 const socketRef = useRef(null);
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
      console.log('✅ WebSocket подключен в чате');
    ...
    socket.on('disconnect', () => {
      console.log('❌ WebSocket отключен в чате');
    });

    // 📨 ПОЛУЧЕНИЕ НОВЫХ СООБЩЕНИЙ ОТ ДРУГИХ
      ...
      if (messageData.chatId === id) {
        setChat(prev => ({
          ...
          messages: [...prev.messages, {
            ...
            fromMe: messageData.senderId === 'current-user-id' // ← замени на реальный ID
          }]
        }));
      }
    });
  ...
  const sendMessage = () => {
    ...
    socketRef.current.emit('send_message', {
      ...
      senderId: 'current-user-id', // ← ЗАМЕНИ НА РЕАЛЬНЫЙ ID ИЗ АВТОРИЗАЦИИ
      senderName: 'Текущий пользователь' // ← ЗАМЕНИ НА РЕАЛЬНОЕ ИМЯ
    });
  ...
  // 🔄 ОБНОВЛЕНИЕ ЧАТА (pull-to-refresh)
...
const styles = StyleSheet.create({
  ...
  // Стили для refresh спиннера (идентично index.js)
  refreshContainer: {
    position: 'absolute',
    top: 100,
    left: 0,
